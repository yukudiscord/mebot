...
client.on('guildMemberAdd', member => {
  ...
  var embed = new Discord.RichEmbed()
    ...
    .setDescription('Привет тебе, ты на сервере Industrial Server!\nИменно этот сервер сделан для общения людей, нахождения Пати в играх. Тут ты можешь найти интересы на разные темы такие как "Игры, общение" и многое другое. На этом сервере администрация дружелюбна, и никому плохого не желает.\nРоли сервера\nТут есть множество ролей, количество которых увеличивается, ну или же уменьшается. Примеры этих ролей: Игровые (По таким играм как Geometry Dash, CS:GO, Minecraft, Fortnite, Terraria) ; Платные (Миллиардер, бизнесмен, славный малый) которые покупаются за реальные деньги ;  Фановые (Роли сделанные просто так, или же с каким нибудь правом, мутом, киком и т.д.)\nБот сервера\nСделанный двумя людьми @Зан и @H2o^. Именно им наш сервер процветает таким крутым ботом. Спасибо им огромное.')
...
        .setColor(c)
        .setTitle("MeBOT")
        .setDescription("Данный бот был создан командой YUKU")
        .addField("OTHER","`is!embed - сказать сообщение embed`\n`is!say` - сказать что-то от именни бота\n`is!ping` - проверить нагрузку на бота\n`is!report` [Юзер] <ПРИЧИНА> - написать репорт на человека")
        .addField("MODER","`is!kick` [Юзер] <ПРИЧИНА> - кикнуть пользователя\n`is!ban` [Юзер] <ПРИЧИНА> - забанить пользователя\n`is!report` [Юзер] <ПРИЧИНА> - написать жалобу\n`is!server` - проверить информацию про данный сервер\n`is!mute` [Юзер] <ВРЕМЯ> - замутить пользователя")
        .addField("FUN","`is!hi` [Юзер] - сказать привет человеку\n`is!hi` - сказать всем привет!\n`is!sad` - уйти в печаль\n`is!angry` [Юзер] - злится на человека\n`is!angry` - начать злится\n`is!sleep` - пойти спать\n`is!suicide` - умереть\n`is!smoke` - покурить сигу")
        try {
...
    if(Kmember.hasPermission("KICK_MEMBERS")) return message.channel.send("У тебя нет прав!");
...
    let incidentchannel = message.guild.channels.find(`name`, "спам-логи");
    if(!incidentchannel) return message.channel.send("Я не могу найти канал `logs`");
...
    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("Прав нету у тебя!");
    if(bUser.hasPermission("BAN_MEMBERS")) return message.channel.send("У тебя нет прав!");
    const banEmbed = new Discord.RichEmbed()
    .setDescription("Был забанен!")
    .setColor(c)
...
    if(!rUser) return message.channel.send("Нет такого");
...
    if(!reportschannel) return message.channel.send("Я не нашёл канал `report`");
...
  if(!tomute) return message.reply("Нет такого");
...
  message.reply(`был замучен на **${ms(ms(mutetime))}**`);
  setTimeout(function(){
